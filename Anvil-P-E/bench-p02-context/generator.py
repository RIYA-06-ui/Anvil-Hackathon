...
def generate(cfg: GenConfig | None = None) -> Dataset:
    ...
    # Sort signals and truth together so they remain aligned
    sig_and_truth = sorted(zip(signals, truth), key=lambda x: x[0]["ts"])
    signals = [x[0] for x in sig_and_truth]
    truth = [x[1] for x in sig_and_truth]
